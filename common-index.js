...
function typeWriter() {
    ...
    if(!el) return;
    if(typeIndex < textToType.length) { el.textContent += textToType.charAt(typeIndex); typeIndex++; typeTimer=setTimeout(typeWriter,80); }
}
function setTypedText(text){const el=document.getElementById('typed-text');if(!el)return;clearTimeout(typeTimer);el.textContent='';textToType=text;typeIndex=0;typeWriter();}
typeWriter();
printLog('系统初始化', '正在拉取动漫一言...', false, 'system');
fetchJsonWithTimeout('https://v1.hitokoto.cn/?c=a&c=b',5000).then(data=>{
    if(!data||!data.hitokoto)throw new Error('一言数据为空');
    setTypedText(`${data.hitokoto} — ⌈${data.from||'一言'}⌋`);
}).catch(e=>{printLog('一言加载失败，使用本地文案',e.message||e,false,'system');setTypedText('心之所向，素履以往。');});
...
async function resolveChineseCity(pos){try{const url='https://api.bigdatacloud.net/data/reverse-geocode-client?'+new URLSearchParams({latitude:pos.latitude,longitude:pos.longitude,localityLanguage:'zh'});const d=await fetchJsonWithTimeout(url,6000);let city=d.city||d.locality||d.principalSubdivision||d.countryName;if(city){city=String(city).trim();pos.city=/[市区县州都道府旗盟]$/.test(city)||!/^[\u3400-\u9fff]+$/.test(city)?city:city+'市';}}catch(e){printLog('中文城市名解析失败',e.message,false,'weather');}return pos;}
async function loadWeather() {
    ...
    try{
        ...
        pos=await resolveChineseCity(pos);
        ...
        const updateTime=new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit',hour12:false});
        widget.innerHTML=`<div class="weather-card-inner" style="position:relative;isolation:isolate;display:flex;align-items:center;gap:14px;width:100%;padding:4px 2px;overflow:hidden;background:linear-gradient(135deg,rgba(255,255,255,.1),rgba(125,180,255,.06));"><span aria-hidden="true" style="position:absolute;z-index:-1;right:-28px;top:-45px;width:95px;height:95px;border-radius:50%;background:radial-gradient(circle,rgba(255,205,93,.26),rgba(255,205,93,0) 68%);"></span><span aria-hidden="true" style="position:absolute;z-index:-1;left:35%;bottom:-55px;width:120px;height:80px;border-radius:50%;background:radial-gradient(circle,rgba(88,160,255,.16),rgba(88,160,255,0) 70%);"></span><div class="weather-icon" style="width:58px;height:58px;flex:none;display:grid;place-items:center;border:1px solid rgba(255,255,255,.3);border-radius:18px;background:linear-gradient(145deg,rgba(255,205,93,.38),rgba(96,165,250,.24));box-shadow:inset 0 1px 0 rgba(255,255,255,.42),0 7px 20px rgba(30,70,120,.13);"><i class="fas ${weather[1]}" style="font-size:27px;filter:drop-shadow(0 3px 5px rgba(0,0,0,.18));"></i></div><div class="weather-info" style="min-width:0;flex:1;"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;"><div class="weather-temp" style="font-size:28px;font-weight:800;line-height:1;letter-spacing:-1px;text-shadow:0 2px 8px rgba(0,0,0,.08);">${temp}<span style="font-size:15px;font-weight:650;vertical-align:top;letter-spacing:0;">°C</span></div><div class="weather-city" title="${escapeHtml(pos.city)}" style="max-width:120px;padding:3px 7px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border-radius:999px;background:rgba(255,255,255,.12);font-size:12px;font-weight:600;"><i class="fas fa-map-marker-alt" style="margin-right:4px;color:#fb7185;"></i>${escapeHtml(pos.city)}</div></div><div class="weather-desc" style="margin-top:4px;font-size:14px;font-weight:700;letter-spacing:.5px;">${weather[0]}</div><div style="display:flex;align-items:center;gap:7px;margin-top:7px;"><span style="display:flex;align-items:center;gap:4px;padding:4px 8px;border:1px solid rgba(255,125,100,.18);border-radius:9px;background:linear-gradient(135deg,rgba(255,135,105,.2),rgba(255,190,120,.08));font-size:13px;font-weight:750;line-height:1;"><i class="fas fa-arrow-up" style="font-size:9px;color:#fb7185;"></i>${high}°</span><span style="display:flex;align-items:center;gap:4px;padding:4px 8px;border:1px solid rgba(75,145,255,.18);border-radius:9px;background:linear-gradient(135deg,rgba(80,150,255,.2),rgba(100,210,255,.08));font-size:13px;font-weight:750;line-height:1;"><i class="fas fa-arrow-down" style="font-size:9px;color:#60a5fa;"></i>${low}°</span><span style="margin-left:auto;white-space:nowrap;font-size:10px;opacity:.62;"><i class="far fa-clock" style="margin-right:3px;"></i>${updateTime}</span></div></div></div>`;
