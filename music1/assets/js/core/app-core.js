...
(function(){
      // Mobile performance mode: reduce expensive effects early
      const __mobilePerf = window.matchMedia('(max-width:860px)').matches;
      if(__mobilePerf){
        document.documentElement.classList.add('mobile-mode');
      }

      // Runtime performance helpers: keep expensive work away from the main thread.
      const perfTools={
        idle(fn,timeout=800){
  ...
  try {
    ...
    function kuwoAudioCandidates(rawUrl){
      const raw=String(rawUrl||'').trim();
      if(!raw)return [];
      const out=[];
      const push=u=>{if(u&&!out.includes(u))out.push(u);};
      // HTTPS 页面只能播放 HTTPS 媒体。酷我 kw-er/kw-lv CDN 支持 HTTPS，
      // 但不要让通用 URL 清洗逻辑丢失接口返回的完整签名查询串。
      if(location.protocol==='https:'&&/^http:\/\//i.test(raw))push(raw.replace(/^http:/i,'https:'));
      else push(raw);
      const normalized=cyNormalizeMediaUrl(raw,'audio');
      push(normalized);
      return out;
    }

    // 酷我使用同一个 <audio> 元素播放。切歌时必须取消上一首残留的媒体监听，
    ...
    function cancelKuwoAudioWait(){
      if(typeof kuwoAudioWaitCancel==='function'){
        try{kuwoAudioWaitCancel();}catch(_){}
      }
      kuwoAudioWaitCancel=null;
    }

    function beginKuwoPlaybackSession(){
      cancelKuwoAudioWait();
      kuwoPlaybackSession+=1;
      return kuwoPlaybackSession;
    }

    function waitKuwoAudioReady(audio,sessionId,requestToken,timeout=6500){
      return new Promise((resolve,reject)=>{
        let done=false;
        const isCurrent=()=>sessionId===kuwoPlaybackSession&&requestToken===state.playRequestToken;
        const cleanup=()=>{
          clearTimeout(timer);
          audio.removeEventListener('canplay',onReady);
          audio.removeEventListener('loadedmetadata',onReady);
          audio.removeEventListener('error',onError);
          if(kuwoAudioWaitCancel===cancel)kuwoAudioWaitCancel=null;
        };
        const finish=(ok,err)=>{
          if(done)return;
          done=true;
          cleanup();
          ok?resolve():reject(err||new Error('Kuwo audio load failed'));
        };
        const cancel=()=>finish(false,Object.assign(new Error('Kuwo playback cancelled'),{name:'AbortError'}));
        const onReady=()=>{
        ...
        const onError=()=>{
          if(!isCurrent())return cancel();
          finish(false,new Error(`Kuwo media error ${audio.error?.code||''}`));
        };
        const timer=setTimeout(()=>{
          if(!isCurrent())return cancel();
          finish(false,new Error('Kuwo media load timeout'));
        },timeout);
        cancelKuwoAudioWait();
        kuwoAudioWaitCancel=cancel;
    ...
    async function playKuwoWithFallback(track,requestToken){
      ...
      if(!isCurrent())return false;
      console.error('[Kuwo] 所有音质均播放失败',lastError);
      return false;
    }
    ...
    let lyricHighlightPending=false;
    let lyricHighlightTime=0;
    function updateLyricsHighlight(time){
      lyricHighlightTime=time;
      if(lyricHighlightPending)return;
      lyricHighlightPending=true;
      requestAnimationFrame(()=>{
        lyricHighlightPending=false;
        const time=lyricHighlightTime;
    ...
    function setupParticles(){
      ...
      const parts=[];
      const N=(reducedMotion||saveData)?12:(mobileParticles?10:56);
      for(let i=0;i<N;i++){
        parts.push({
    ...
    function setupEvents(){
      ...
      const clearSearchState=perfTools.debounce(()=>{
      ...
      dom.searchInput.addEventListener('input',clearSearchState);
